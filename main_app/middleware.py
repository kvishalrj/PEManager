    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            if user.user_type == '1': # Is it the HOD/Admin
                if modulename == 'main_app.projectEngineer_views':
                    return redirect(reverse('admin_home'))
            elif user.user_type == '2': #  Manager :-/ ?
                if modulename == 'main_app.projectEngineer_views' or modulename == 'main_app.hod_views':
                    return redirect(reverse('manager_home'))
            elif user.user_type == '3': # ... or Project Engineer ?
                if modulename == 'main_app.hod_views' or modulename == 'main_app.manager_views':
                    return redirect(reverse('projectEngineer_home'))
